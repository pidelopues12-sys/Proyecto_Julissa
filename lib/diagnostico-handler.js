...
export function createHandler(client, options = {}) {
  const model =
    String(options.model || process.env.NVIDIA_MODEL || "openai/gpt-oss-20b")
      .trim()
      .replace(/^["']+|["']+$/g, "")
      .trim() || "openai/gpt-oss-20b";
  const maxTokens = Number(options.maxTokens || process.env.NVIDIA_MAX_TOKENS || 2048);
  return async function handler(req, res) {
    ...
    try {
      const completion = await client.chat.completions.create({
        ...
        // gpt-oss es un modelo de razonamiento; esfuerzo bajo = respuesta más
        // rápida (evita agotar el tiempo de la función) sin perder utilidad.
      ...
      return res.status(502).json({
        error: "No se pudo generar el diagnóstico. Intenta más tarde.",
        _debug: { reason: "exception", model, base: options.baseURL, status: err?.status ?? null, message: String(err?.message ?? err).slice(0, 300) },
      });
    }
  };
}
